...
// import rasm from '../../public/ummiH.webp'
import rasm from '../../public/ImgUmmi.webp';
...
const Hero = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const phoneInputElement = document.getElementById("hero-number") as HTMLInputElement | null;

    if (!phoneInputElement) {
        toast.error("Phone input field is missing.");
        return;
    }

    const phoneNumber = phoneInputElement.value.trim();
    const phoneRegex = /^(\+998|998)[\s\-]?[1-9][0-9]{8}$/; // Adjust regex for Uzbek phone numbers
    const MessageFull = `Tezkor qo'ng'iroq uchun nomer: ${phoneNumber}`;
    ...
    try {
        ...
        if (response.ok) {
            ...
            toast.success("Your number has been sent, we will contact you soon...");
        } else {
    ...
    <section className='text-gray-600 body-font'>
      <div className='container mx-auto flex px-2 py-24 md:flex-row flex-col items-center'>
        <div className='lg:max-w-lg lg:w-full md:w-1/2 w-5/6 mb-10 md:mb-0'>
          <Image
            className='object-cover object-center w-full rounded h-[25rem]'
            alt='hero'
            src={rasm}
            priority
          />
        ...
        <div className='lg:flex-grow md:w-1/2 lg:pl-24 md:pl-16 flex flex-col md:items-start md:text-left items-center text-center'>
          ...
          We are truly grateful for the trust and support of our esteemed clients!
          </h1>
          <p className='mb-8 leading-relaxed'>
            If you would like to know more now, please feel free to share your phone number so that we can respond quickly.
          </p>
            <form
            action="method "
            id='myForm' 
